...
%% Load fast data (cycle-resolved pressure and crank angle)
% Fast data file path
fdaqFile = fullfile('Data', 'Example', '20251120_0000001_example_fdaq.txt');
...
Ncycles = Nrows/NdatapointsperCycle;              % This must be an integer. If not check what is going on

% Reshape data into matrices
Ca = reshape(dataIn(:,1), [], Ncycles);           % [deg] - Both p and Ca are now matrices of size (NCa, Ncycles)
p  = reshape(dataIn(:,2), [], Ncycles) * bara;    % [Pa] - Convert from bar to Pa

%% Load slow data (averaged measurements)
sdaqFile = fullfile('Data', 'Example', '20251120_0000001_example_sdaq.txt');
slowData = readmatrix(sdaqFile);
p_intake_bar = slowData(end, 4);  % intake pressure [bar(a)] from column 4
p_intake = p_intake_bar * 1e5;    % [Pa]

%% Plotting cycle 10 
f1 = figure(1);           
pp = plot(Ca, p/bara, 'LineWidth', 1);           
xlabel('Ca [deg]'); ylabel('p [bar]');           
xlim([-360 360]); ylim([0 50]);                  
iselect = 10;                                     
line(Ca(:,iselect), p(:,iselect)/bara, 'LineWidth', 2, 'Color', 'r');
YLIM = ylim;
% Add some extras to the plot
line([CaIVC CaIVC], YLIM, 'LineWidth', 1, 'Color', 'b'); % Plot a vertical line at IVC
line([CaEVO CaEVO], YLIM, 'LineWidth', 1, 'Color', 'r'); % Plot a vertical line at EVO
...
xlabel('V [dm^3] (logarithmic scale)'); ylabel('p [bar] (logarithmic scale)');
xlim([0.02 0.8]); ylim([0.5 50]);
...
%% Calculate Work from pV-diagram for selected cycle
...
%% Calculate Work for ALL cycles with pegging and filtering
...
fprintf('\n=== Work Calculation for ALL %d Cycles (with Pegging & Filtering) ===\n', Ncycles);
...
%% Plot averaged and filtered pressure
f5 = figure(5);
plot(workResults.Ca_avg, workResults.p_avg/bara, 'b-', 'LineWidth', 1, 'DisplayName', 'Averaged (pegged)');
hold on;
plot(workResults.Ca_avg, workResults.p_filt/bara, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Filtered');
hold off;
xlabel('Ca [deg]');
ylabel('p [bar]');
xlim([-360 360]);
ylim([0 50]);
set(gca, 'XTick', [-360:60:360], 'XGrid', 'on', 'YGrid', 'on');
title('Average Pressure: Raw vs Filtered (Pegged at BDC)');
