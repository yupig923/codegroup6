...
%% pV-diagram Compare Cos with Modified Cos
[V1,V2] = CylinderVolume(Ca(:,iselect),Cyl);
f2 = figure(2);
subplot(1,2,1)
hold on
plot(V1/dm^3,p(:,iselect)/bara,"Color","r");
plot(V2/dm^3,p(:,iselect)/bara,"Color","g");
hold off
legend(["Cosine","Modified Cosine"])
...
title({'pV-diagram'})
subplot(1,2,2)
loglog(V1/dm^3,p(:,iselect)/bara,"r",V2/dm^3,p(:,iselect)/bara,"g");
legend(["Cosine","Modified Cosine"])
xlabel('V [dm^3]');ylabel('p [bar]');               % Always add axis labels
...
f3 = figure(3);
subplot(1,2,1)
plot(V2/dm^3,p(:,iselect)/bara,"Color","#00FFFF");
xlabel('V [dm^3]');ylabel('p [bar]');               % Always add axis labels
xlim([0 0.8]);ylim([0.5 50]);                      % Matter of taste
set(gca,'XTick',[0:0.1:0.8],'XGrid','on','YGrid','on');        % I like specific axis labels. Matter of taste
title({'pV-diagram'})
subplot(1,2,2)
loglog(V2/dm^3,p(:,iselect)/bara,"Color","#00FFFF");
xlabel('V [dm^3]');ylabel('p [bar]');               % Always add axis labels
xlim([0.02 0.8]);ylim([0 50]);                      % Matter of taste
set(gca,'XTick',[0.02 0.05 0.1 0.2 0.5 0.8],...
    'YTick',[0.5 1 2 5 10 20 50],'XGrid','on','YGrid','on');        % I like specific axis labels. Matter of taste
